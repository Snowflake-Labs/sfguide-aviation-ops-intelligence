...
| 1b | App Runtime dashboard | `SHOW APPLICATION SERVICES LIKE 'AVIATION_DASHBOARD' IN SCHEMA SNOWFLAKE_APPS.PUBLIC` (by name — Application Services may lack COMMENT). Or `snow app teardown`. | `DROP APPLICATION SERVICE IF EXISTS SNOWFLAKE_APPS.PUBLIC.AVIATION_DASHBOARD;` then `DROP ARTIFACT REPOSITORY IF EXISTS SNOWFLAKE_APPS.PUBLIC.AVIATION_DASHBOARD_REPO;` (leave shared `SNOWFLAKE_APPS` DB / `SNOWFLAKE_APPS_QUERY_WH` unless unused) |
| 1c | SPCS dashboard (fallback path) | `SHOW SERVICES LIKE 'AVIATION_DASHBOARD_SERVICE' IN ACCOUNT` (by name) | `DROP SERVICE IF EXISTS <db>.PUBLIC.AVIATION_DASHBOARD_SERVICE;` then `DROP COMPUTE POOL IF EXISTS AVIATION_DASHBOARD_COMPUTE_POOL;` `DROP IMAGE REPOSITORY IF EXISTS <db>.PUBLIC.AVIATION_DASHBOARD_REPO;` |
